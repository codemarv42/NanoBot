...
void CalibrateTCS34725(int anz) { //The RGB sensors are calibrated with green
  ShiftRegisterWrite(SR_LED_PT, HIGH);
  ShiftRegisterWrite(SR_LED_RGB, HIGH);
  ...
  for (int n = 0; n < anz; n++) {
    ...
    for (int i = 0; i < 5; i++) {
      if (RAW_TCS_L_G_MAX[i] < RAW_TCS_L[i]) {
        RAW_TCS_L_G_MAX[i] = RAW_TCS_L[i];
      }
      if (RAW_TCS_L_G_MIN[i] > RAW_TCS_L[i]) {
        RAW_TCS_L_G_MIN[i] = RAW_TCS_L[i];
      }
    }

    for (int i = 0; i < 5; i++) {
      if (RAW_TCS_R_G_MAX[i] < RAW_TCS_R[i]) {
        RAW_TCS_R_G_MAX[i] = RAW_TCS_R[i];
      }
      if (RAW_TCS_R_G_MIN[i] > RAW_TCS_R[i]) {
        RAW_TCS_R_G_MIN[i] = RAW_TCS_R[i];
      }
    }
