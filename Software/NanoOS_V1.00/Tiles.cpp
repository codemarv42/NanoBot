...
void CaseDistinction() {  //Tiles case distinction
  ...
  } else if ((CAL_L[L_A] > White) && (CAL_L[M] > White) && (CAL_L[R_A] > White) && (CAL_L[L_A2] > White - 70) && (CAL_L[R_A2] > White - 70)) {  //When all sensors see white
...
void FindExit() {  //Find the exit in Rescue area
  ...
  if (DistL < DistR) {  //turn right
    ...
    while ((CAL_L[L_A] > Black + 100) && (CAL_L[M] > Black + 100) && (CAL_L[R_A] > Black + 100)) {
      ...
      if (DistL > 160) {  //An Aus/Eingang vorbeifahren
        RotateAngle(5);
        while (DistL > 160) {
          Forward(OUT_AB, Slow + 15);
        }
      }
    }
  } else {  //turn left
