...
void VL53l0xInit(int Port) {  //The TOF sensors are initalised (TOF - Time of flight) //Distance sensors
  TCASelect(Port);
...
bool TCSL_Red() {                                                                      //True - red, False - No red //Looks if it is the calibrated color
  if ((RAW_TCS_L[0] <= RAW_TCS_L_R_MAX[0]) && (RAW_TCS_L[0] >= RAW_TCS_L_R_MIN[0])) {  //Yes, that is extreamly ugly
    if ((RAW_TCS_L[1] <= RAW_TCS_L_R_MAX[1]) && (RAW_TCS_L[1] >= RAW_TCS_L_R_MIN[1])) {
      if ((RAW_TCS_L[2] <= RAW_TCS_L_R_MAX[2]) && (RAW_TCS_L[2] >= RAW_TCS_L_R_MIN[2])) {
        if ((RAW_TCS_L[3] <= RAW_TCS_L_R_MAX[3]) && (RAW_TCS_L[3] >= RAW_TCS_L_R_MIN[3])) {
          if ((RAW_TCS_L[4] <= RAW_TCS_L_R_MAX[4]) && (RAW_TCS_L[4] >= RAW_TCS_L_R_MIN[4])) {
            return true;
          } else {
            return false;
          }
...
bool TCSR_Red() {                                                                      //True - red, False - No red //Looks if it is the calibrated color
  if ((RAW_TCS_R[0] <= RAW_TCS_R_R_MAX[0]) && (RAW_TCS_R[0] >= RAW_TCS_R_R_MIN[0])) {  //Yes, that is extreamly ugly
    if ((RAW_TCS_R[1] <= RAW_TCS_R_R_MAX[1]) && (RAW_TCS_R[1] >= RAW_TCS_R_R_MIN[1])) {
      if ((RAW_TCS_R[2] <= RAW_TCS_R_R_MAX[2]) && (RAW_TCS_R[2] >= RAW_TCS_R_R_MIN[2])) {
        if ((RAW_TCS_R[3] <= RAW_TCS_R_R_MAX[3]) && (RAW_TCS_R[3] >= RAW_TCS_R_R_MIN[3])) {
          if ((RAW_TCS_R[4] <= RAW_TCS_R_R_MAX[4]) && (RAW_TCS_R[4] >= RAW_TCS_R_R_MIN[4])) {
            return true;
          } else {
            return false;
          }
...
void SetTCS_Green(bool DATASET) {  //DATASET: A/B, 0/1 //The calibrated green values are set manually
  if (DATASET == 0) {              //DATASET: A
    ...
    RAW_TCS_L_G_MAX[R] = 4;
    RAW_TCS_L_G_MAX[G] = 9;
    RAW_TCS_L_G_MAX[B] = 14;
    RAW_TCS_L_G_MAX[C] = 25;
    RAW_TCS_L_G_MAX[L] = 5;

    RAW_TCS_L_G_MIN[R] = 0;
    RAW_TCS_L_G_MIN[G] = 5;
    RAW_TCS_L_G_MIN[B] = 11;
    RAW_TCS_L_G_MIN[C] = 17;
    RAW_TCS_L_G_MIN[L] = 1;

    RAW_TCS_R_G_MAX[R] = 3;
    RAW_TCS_R_G_MAX[G] = 10;
    RAW_TCS_R_G_MAX[B] = 15; //Hohe Streuung 11-20
    RAW_TCS_R_G_MAX[C] = 28; //Hohe Streuung 21-38
    RAW_TCS_R_G_MAX[L] = 5;

    RAW_TCS_R_G_MIN[R] = 0;
    RAW_TCS_R_G_MIN[G] = 5;
    RAW_TCS_R_G_MIN[B] = 7;
    RAW_TCS_R_G_MIN[C] = 16;
    RAW_TCS_R_G_MIN[L] = 1;
