...
unsigned int TilesCase = 0;  //0 - Follow line, 1 - Left, 2 - Right, 3 - Gap, 4 - Left Green, 5 - Right Green, 6 - Turn, 7 - End, 8 - Obstacle, 9 - Follow line up, 10 - Follow line down, 12 - Rescue, 13 - Crossing, 14 - Wumper
...
void CaseDistinction() {  //Tiles case distinction
  ...
  /*
  if (abs(XAccel) > 21){  //Wumper detected
    OnWumper = true;
  }else{
    if (OnWumper == true){
      OnWumper = false;
      WumperTime = millis() + 2000;
    }
  }
  */
}
