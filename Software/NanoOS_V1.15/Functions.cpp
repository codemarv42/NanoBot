...
void Loop1Code(void* pvParameters) {  //Task of a processor core 0
  ...
  for (;;) {
    if (ENMainProgram == 0) {
      ...
      if ((ENCButtonState == 1) && (ENShutdownBT == 1)) {
...
void EepromSave() {  //Saves the values in the EEPROM
  ...
  for (int i = 14; i < 28; i += 2) {
    writeUnsignedIntIntoEEPROM(i, RAW_MAX[(i - 14) / 2]);
  }
...
void EepromLoad() {  //Read the data from the EEPROM and set the variables accordingly
  ...
  for (int i = 14; i < 28; i += 2) {
    RAW_MAX[(i - 14) / 2] = readUnsignedIntFromEEPROM(i);
    if (RAW_MAX[(i - 14) / 2] == 65535) {
      EEPROMValid = false;
    }
  }
